...
class ChemistRepository {
  ...
  Stream<List<Chemist>> watchChemists() => (_db.select(_db.chemists)
        ..where((t) => t.isEnabled.equals(true))
        ..orderBy([(t) => OrderingTerm.desc(t.cdt)]))
      .watch();

  Future<Chemist?> getById(String id) =>
      (_db.select(_db.chemists)..where((t) => t.id.equals(id)))
          .getSingleOrNull();
  ...
  Future<List<Chemist>> dropdownOptions() => (_db.select(_db.chemists)
        ..where((t) => t.isEnabled.equals(true))
  ...
  Future<String> createChemist(Map<String, dynamic> fields) async {
    ...
    await _db.transaction(() async {
      await _db.into(_db.chemists).insert(ChemistsCompanion.insert(
            id: id,
            fullName: Value((fields['full_name'] ?? '') as String),
            phone: Value(fields['phone'] as String?),
            email: Value(fields['email'] as String?),
            contactPersonName: Value(fields['contact_person_name'] as String?),
            contactPersonEmail:
                Value(fields['contact_person_email'] as String?),
  ...
  Future<void> updateChemist(String id, Map<String, dynamic> fields) async {
    ...
    final String now = DateTime.now().toUtc().toIso8601String();
    await _db.transaction(() async {
      await (_db.update(_db.chemists)..where((t) => t.id.equals(id)))
